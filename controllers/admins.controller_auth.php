	...
	public function securityCode(){
		...
		if(isset($_POST["scode_admin"])){
			...
			if($admin->status == 200){
				...
				if($admin->results[0]->id_office_admin > 0){

					$url = "relations?rel=admins,offices&type=admin,office&linkTo=id_admin,id_office&equalTo=".$admin->results[0]->id_admin.",".$admin->results[0]->id_office_admin;
					$method = "GET";
					$fields = array();
