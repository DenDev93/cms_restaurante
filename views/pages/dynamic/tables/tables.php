...
if($module->title_module == "offices" && $_SESSION["admin"]->id_office_admin > 0){
	...
	/*if($_SESSION["admin"]->id_office_admin == 0 || 
		...
		if($module->title_module == "offices" && isset($_SESSION["admin"]->id_office_admin) && $_SESSION["admin"]->id_office_admin > 0){
		...
		$id_office = isset($_SESSION["admin"]->id_office_admin) ? $_SESSION["admin"]->id_office_admin : 0;

		if($id_office == 0 || !in_array("id_office_".$module->suffix_module, array_column($module->columns, "title_column"))){

			$url = $module->title_module."?orderBy=id_".$module->suffix_module."&orderMode=DESC&startAt=0&endAt=".$limit;

		}else{

			$url = $module->title_module."?orderBy=id_".$module->suffix_module."&orderMode=DESC&startAt=0&endAt=".$limit."&linkTo=id_office_".$module->suffix_module."&equalTo=".$id_office;
...
echo '<div class="form-check form-switch">
											<input class="form-check-input px-3 changeBoolean" type="checkbox" id="mySwitch" '.$checked.' idItem="'.base64_encode($value["id_".$module->suffix_module]).'" table="'.$module->title_module.'" suffix="'.$module->suffix_module.'" column="'.$item->title_column.'">
											<label class="form-check-label ps-1 align-middle" for="mySwitch">'.$label.'</label>
											</div>';

										}else{

											echo '<span class="form-check-label ps-1 align-middle">'.$label.'</span>';
										}
										...
										=============================================*/
									    	...
									    	echo 'C$ '.number_format(urldecode($value[$item->title_column]),2);
										...
										=============================================*/
											...
											if($item->matrix_column != null && isset($value[$item->title_column]) && $value[$item->title_column] > 0){
												...
												if($getModule->status == 200 && !empty($getModule->results) && is_object($getModule->results[0])){

													$urlPage = $getModule->results[0]->url_page;
													$suffixModule = $getModule->results[0]->suffix_module;
													...
													if($relation->status == 200 && !empty($relation->results) && is_object($relation->results[0])){
										...
										}else if($item->type_column == "html"){


											echo urldecode($value[$item->title_column]);
